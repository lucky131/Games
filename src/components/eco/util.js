export default {
  ...
  methods: {
    linearColorRGB(x1,r1,g1,b1,x2,r2,g2,b2,x,gap) {
      let rgb={};
      gap=gap||1;
      x=x-x%gap;
    ...
    inverseColorRGB(x1,r1,g1,b1,r2,g2,b2,flag,x,gap) {
      gap=gap||1;
      x=x-x%gap;
      return "rgb("+
        Math.floor(r2-x1*(r2-r1)/((x-x1)/flag+x1))+
        ","+
        Math.floor(g2-x1*(g2-g1)/((x-x1)/flag+x1))+
        ","+
        Math.floor(b2-x1*(b2-b1)/((x-x1)/flag+x1))+
        ")";
    },
